<?php
include 'koneksi.php';
$card = mysqli_query($koneksi, "SELECT * FROM potensi WHERE id = '" . $_GET['id'] . "'");
...
<body>
    ...
    <div class="imgatas">
        <img src="asset/img/img_paralax.jpg" alt="">
    </div>

    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="#">Home</a></li>
            <li class="breadcrumb-item"><a href="potensi.php">Potensi</a></li>
            <li class="breadcrumb-item active" aria-current="page">Detail Potensi</li>
        </ol>
    </nav>
    ...
    <div class="container11">
        <div class="bodyisi">
            ...
            <div class="isidetail">
                <div class="jdl">Potensi Desa Lainnya</div>
                <div class="samping">
                    <?php $card = mysqli_query($koneksi, "SELECT * FROM wisata WHERE id NOT IN ('" . $_GET['id'] . "') ORDER BY RAND() LIMIT 4 ");
                    if (mysqli_num_rows($card) > 0) {
                        while ($p = mysqli_fetch_array($card)) { ?>
                            <div class="boxsamping">
                                <div class="pic"><a title="<?= $p['judul'] ?>" href="detailpotensi.php?id=<?= $p['id'] ?>"><img src="admin/img/<?= $p['gambar'] ?>" alt=""></a></div>
                                <div class="kanan">
                                    <?php
                                    $kata = $p['judul'];
                                    $jml = strlen($kata);
                                    if ($jml <= 45) { ?>
                                        <a title="<?= $p['judul'] ?>" href="detailpotensi.php?id=<?= $p['id'] ?>"><?= substr($p['judul'], 0, 45) ?></a>
                                    <?php } else if ($jml > 45) { ?>
                                        <a title="<?= $p['judul'] ?>" href="detailpotensi.php?id=<?= $p['id'] ?>"><?= substr($p['judul'], 0, 45) ?>...</a>
                                    <?php } ?>
                                    <div class="postedBy">
                                         <a class="btn btn-primary" href="checkwisata.php" role="button">Check Wisata</a>
                                    </div>
                                </div>
                            </div>
                        <?php }
