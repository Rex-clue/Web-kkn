<?php
include 'koneksi.php';
$card = mysqli_query($koneksi, "SELECT * FROM pertanian WHERE id = '" . $_GET['id'] . "'");
...
<head>
    ...
    <title>Detail Pertanian Desa Gunungsari</title>
...
<body>
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="pertanian.php">Pertanian</a></li>
            <li class="breadcrumb-item active" aria-current="page">Detail Pertanian</li>
        </ol>
    </nav>
    ...
    <div class="container11">
        <div class="bodyisi">
            ...
            <div class="isidetail">
                <div class="jdl">Pertanian Desa Lainnya</div>
                <div class="samping">
                    <?php $card = mysqli_query($koneksi, "SELECT * FROM pertanian WHERE id NOT IN ('" . $_GET['id'] . "') ORDER BY RAND() LIMIT 4 ");
                    if (mysqli_num_rows($card) > 0) {
                        while ($p = mysqli_fetch_array($card)) { ?>
                            <div class="boxsamping">
                                <div class="pic"><a title="<?= $p['judul'] ?>" href="detailpertanian.php?id=<?= $p['id'] ?>"><img src='../img/'<?= $p['gambar'] ?> alt=""></a></div>
                                <div class="kanan">
                                    <?php
                                    $kata = $p['judul'];
                                    $jml = strlen($kata);
                                    if ($jml <= 45) { ?>
                                        <a title="<?= $p['judul'] ?>" href="detailpertanian.php?id=<?= $p['id'] ?>"><?= substr($p['judul'], 0, 45) ?></a>
                                    <?php } else if ($jml > 45) { ?>
                                        <a title="<?= $p['judul'] ?>" href="detailpertanian.php?id=<?= $p['id'] ?>"><?= substr($p['judul'], 0, 45) ?>...</a>
                                    <?php } ?>
    ...
    <script src="pertanian.js"></script>
</body>
