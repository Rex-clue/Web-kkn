<?php
include 'koneksi.php';
$card = mysqli_query($koneksi, "SELECT * FROM umkm WHERE id = '" . $_GET['id'] . "'");
...
<body>
    ...
    <div class="container11">
        <div class="bodyisi">
            ...
            <div class="isidetail">
                <div class="jdl">UMKM Desa Lainnya</div>
                <div class="samping">
                    <?php $card = mysqli_query($koneksi, "SELECT * FROM umkm WHERE id NOT IN ('" . $_GET['id'] . "') ORDER BY RAND() LIMIT 4 ");
                    if (mysqli_num_rows($card) > 0) {
                        while ($p = mysqli_fetch_array($card)) { ?>
                            <div class="boxsamping">
                                <div class="pic"><a title="<?= $p['judul'] ?>" href="detailumkm.php?id=<?= $p['id'] ?>"><img src="../img/<?= $p['gambar'] ?>" alt=""></a></div>
                                <div class="kanan">
                                    <?php
                                    $kata = $p['judul'];
                                    $jml = strlen($kata);
                                    if ($jml <= 45) { ?>
                                        <a title="<?= $p['judul'] ?>" href="detailumkm.php?id=<?= $p['id'] ?>"><?= substr($p['judul'], 0, 45) ?></a>
                                    <?php } else if ($jml > 45) { ?>
                                        <a title="<?= $p['judul'] ?>" href="detailumkm.php?id=<?= $p['id'] ?>"><?= substr($p['judul'], 0, 45) ?>...</a>
                                    <?php } ?>
